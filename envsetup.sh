...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ninja_") ; then
        NinjaOS_BUILD=$(echo -n $1 | sed -e 's/^ninja_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $NinjaOS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        NinjaOS_BUILD=
    fi
    export NinjaOS_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${NinjaOS_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    NinjaOS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/ninja/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the NinjaOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch ninja_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Ninja GitHub
        T=$(gettop)
        cd $T > /dev/null
...
function repopick() {
    T=$(gettop)
    $T/vendor/ninja/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $NinjaOS_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.ninja.device | grep -q "$NinjaOS_BUILD");
    then
        ...
        echo "The connected device does not appear to be $NinjaOS_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.ninja.device | grep -q "$NinjaOS_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/ninja/build/envsetup.sh
